...
<x-card class="bg-white p-4 rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-4">
        ...
        <x-button class="bg-indigo-600 hover:bg-indigo-700">
            <a href="/admin/prestazioni/nuova" class="block text-white">Nuova</a>
        </x-button>
    </div>
        ...
        <x-table :headers="['ID', 'Descrizione', 'Prescrizioni', 'Medico', 'Staff', 'Azioni']">
            @foreach($prestazioni as $prestazione)
                <tr class="hover:bg-indigo-50 transition">
                    ...
                    <td class="px-6 py-3">{{ $prestazione->medico->nome ?? 'N/A' }}</td>
                    <td class="px-6 py-3">{{ $prestazione->staff->name ?? '-' }}</td>
                    <td class="px-6 py-3 flex gap-4 items-center">
                        <a href="/admin/prestazioni/{{ $prestazione->id }}" class="text-indigo-600 hover:text-indigo-800" title="Modifica">
