...
<x-card class="bg-white p-4 rounded-lg shadow-md">
    ...
    @if(isset($prestazioni) && count($prestazione) > 0)
    <x-table :headers="['Prestazione', 'Nome Medico', 'Cognome Medico', 'Azioni']">
        @foreach($prenotazioni as $prenotazione)
            <tr class="hover:bg-indigo-50 transition">
                <td class="px-6 py-3">{{ $prestazione->descrizione }}</td>
                <td class="px-6 py-3 capitalize">{{ $prestazione->medico->nome }}</td>
                <td class="px-6 py-3 capitalize">{{ $prestazione->medico->cognome }}</td>
                <td class="px-6 py-3 flex gap-4 items-center">
                    <a href="/staff/prenotazioni/{{ $prenotazione->id }}" class="text-indigo-600 hover:text-indigo-800" title="Modifica">
